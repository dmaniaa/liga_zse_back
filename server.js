...
const client = new tmi.Client({
  ...
  identity: {
    username: process.env.TWITCH_UNAME,
    password: process.env.TWITCH_TOKEN
  },
  channels: [process.env.TWITCH_UNAME]
});
...
app.options('/api/bot/enableGiveaway', cors(corsOptions), (req, res) => {
  res.status(200)
})
app.post('/api/bot/enableGiveaway', cors(corsOptions), authenticateToken, checkAdmin, async (req, res) => {
  giveaway = true;
  const workingData = {
    error: null
  }
  ...
  try {
    const [rows, fields] = await promisePool.execute('DELETE FROM giveaway')
  }
  catch (err) {
    workingData.error = err
    sendToLog(req.path, err, req.user.uid)
  }
  if (workingData.error) {
    response.status.code = "500";
    response.status.message = "Błąd SQL"
  }
  else {
    response.status.code = "200";
    response.status.message = "Giveaway uruchomiony";
    client.say('zse_esport', `Giveaway został uruchomiony! Wpisz !giveaway aby dołączyć`);
  }
  res.status(response.status.code).send(response);
})

app.options('/api/bot/disableGiveaway', cors(corsOptions), (req, res) => {
  res.status(200)
})
app.post('/api/bot/disableGiveaway', cors(corsOptions), authenticateToken, checkAdmin, (req, res) => {
  giveaway = false;
  const response = {
    status: {
      code: 0,
  ...
  response.status.code = 200;
  response.status.message = "Giveaway zatrzymany";
  client.say('zse_esport', `Giveaway został zatrzymany! Nie można już dołączyć`);
  res.status(response.status.code).send(response);
})

app.options('/api/bot/getGiveaway', cors(corsOptions), (req, res) => {
  res.status(200)
})
app.get('/api/bot/getGiveaway', cors(corsOptions), authenticateToken, checkAdmin, async (req, res) => {
  const workingData = {
    users: null,
    error: null,
  }
  ...
  try {
    const [rows, fields] = await promisePool.execute('SELECT username FROM giveaway;')
    workingData.users = rows
  }
  catch (err) {
    workingData.error = err
    sendToLog(req.path, err, req.user.uid)
  ...
  if (workingData.error) {
    ...
    response.status.code = '404'
    response.status.message = 'Nie znaleziono użytkowników'
    res.status(200).send(response);
  }
  else {
    response.data = workingData.users
    response.status.code = '200'
    response.status.message = 'OK'
    res.status(200).send(response);
  }
...
app.options('/api/bot/enable', cors(corsOptions), (req, res) => {
  res.status(200)
})
app.post('/api/bot/enable', cors(corsOptions), authenticateToken, checkAdmin, (req, res) => {
  bot_enabled = true;
  const response = {
    status: {
      code: 0,
  ...
  response.status.code = '200';
  response.status.message = "Bot uruchomiony";
  client.say('zse_esport', `Bot został uruchomiony! Wpisz !help żeby uzyskać listę ciekawych komend naszego bota`);
  res.status(response.status.code).send(response);
})
app.options('/api/bot/disable', cors(corsOptions), (req, res) => {
  res.status(200)
})
app.post('/api/bot/disable', cors(corsOptions), authenticateToken, checkAdmin, (req, res) => {
  bot_enabled = false;
  const response = {
    status: {
      code: 0,
  ...
  response.status.code = '200';
  response.status.message = "Bot zatrzymany";
  client.say('zse_esport', `Bot został zatrzymany, nauraaaa!`);
  res.status(response.status.code).send(response);
})
...
client.on('message', async (channel, tags, message, self) => {
  ...
  if (message.toLowerCase() === '!giveaway') {
    const workingData = {
      error: null
    }
    if (giveaway) {
      try {
        const [rows, fields] = await promisePool.execute('INSERT INTO giveaway VALUES(?,?);', [tags["user-id"], tags["username"]])
      }
      catch (err) {
        workingData.error = err
        sendToLog(req.path, err)
      ...
      if (workingData.error) {
        client.say(channel, `@${tags.username} już jesteś w giveawayu!`)
      }
      else {
        client.say(channel, `@${tags.username} dodano do giveawaya!`)
      }

    }
    else {
      client.say(channel, `@${tags.username} giveaway nie jest uruchomiony!`);
    }
  }
  if (message.toLowerCase() === '!help') {
    client.say(channel, '!giveaway - weź udział w giveawayu jeśli jest uruchomiony, !solo - wybierz kogoś z kim idziesz na solo, !salto *jakiś tekst* - zrób salto, odwróć tekst');
  }
  if (message.toLowerCase() === '!solo') {
    axios.get('https://tmi.twitch.tv/group/user/zse_esport/chatters')
      .then(function (response) {
        const viewers = response.data.chatters.viewers.concat(response.data.chatters.moderators, response.data.chatters.viewers);
        if (viewers) {
          const random = Math.floor(Math.random() * viewers.length);
          const solo = viewers[random];
          client.say(channel, `@${tags.username} lecisz w solo z @${solo}`);
        }
        else {
          client.say(channel, `@${tags.username} sorki, nie masz z kim lecieć w solo`);
        }
      })
  ...
  if (message.toLowerCase().split(" ")[0] === '!salto') {
    const text = message.split(" ").slice(1).join(' ');
    const reversed = text.split("").reverse().join("");
    client.say(channel, reversed);
  }
});

setInterval(function () {
  if (bot_enabled) client.say('zse_esport', 'Wpisz !help żeby uzyskać listę ciekawych komend naszego bota');
  // do your stuff here
}, 2 * 60 * 1000);
